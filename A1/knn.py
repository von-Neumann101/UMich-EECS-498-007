...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        for j in range(num_test):
            det = x_test_flat[j] - x_train_flat[i]
            dists[i][j] = torch.dot(det, det)
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_test):
        det = x_test_flat - x_train_flat[i]
        dists[i] = (det * det).sum(dim=1)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train_flat = x_train.reshape(num_train, -1)
    x_test_flat = x_test.reshape(num_test, -1) 
    x_train_sq = (x_train_flat * x_train_flat).sum(dim=1).view(num_train, 1)
    x_test_sq = (x_test_flat * x_test_flat).sum(dim=1).view(1, num_test)
    dists = x_train_sq + x_test_sq - 2 * x_train_flat.mm(x_test_flat.t())
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for j in range(num_test):
        """
        固定一个测试集,dists[:, j]就是每个x_train和x_test的距离向量
        我们取小的前k个index,然后传入y_train得到前k近的图像的标签
        然后数谁最多
        """
        _, nearest_idx = torch.topk(dists[:, j], k=k, largest=False)
...
def knn_cross_validate(
    ...
    for i in range(num_folds):
        x_dev = x_train_folds[i]
        y_dev = y_train_folds[i]

        x_tr = torch.cat(x_train_folds[:i] + x_train_folds[i+1:], dim=0)
        y_tr = torch.cat(y_train_folds[:i] + y_train_folds[i+1:], dim=0)

        k_test = KnnClassifier(x_tr, y_tr)
        for k in k_choices:
            acc = k_test.check_accuracy(x_dev, y_dev, k, quiet=True)
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for k in sorted(k_to_accuracies.keys()):
        mean_acc = torch.mean(torch.tensor(k_to_accuracies[k], dtype=torch.float32))
