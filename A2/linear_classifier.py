    ...
    for i in range(num_train):
        ...
        for j in range(num_classes):
            ...
            if margin > 0:
                ...
                # 对于这种，形状的Tensor来说，我们完全不需要考虑其大小为D的维度
                # dW初始为0，我们只需要考虑margin>0的情况——L=\sum margin
                # 我们要求L对W的导数,注意到当分类为其他样本时导数为1，正确样本导数为-1
                # 其中s=wx
                dW[:, j] += X[i]
                dW[:, y[i]] -= X[i]
                #######################################################################
...
def test_one_param_set(
    ...
    y_pred_t = cls.predict(data_dict["X_train"])
    y_pred_v = cls.predict(data_dict["X_val"])
    train_acc = (y_pred_t == data_dict["y_train"]).float().mean().item()
    val_acc = (y_pred_v == data_dict["y_val"]).float().mean().item()
    # Replace "pass" statement with your code
