...
class RPNPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        for out_channel in stem_channels:
            conv = nn.Conv2d(in_channels=prev_channel, out_channels=out_channel, kernel_size= 3, padding=1, stride=1)
        ...
        self.pred_obj = nn.Conv2d(prev_channel, num_anchors, kernel_size=1)
        self.pred_box = nn.Conv2d(prev_channel, 4 * num_anchors, kernel_size=1)
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        # Replace "pass" statement with your code
        for p, feature in feats_per_fpn_level.items():
            stem_out = self.stem_rpn(feature)
            obj = self.pred_obj(stem_out)
            box = self.pred_box.forward(stem_out)
            # (B, _, H, W) -> (B, H, W, _) -> (B, H * W * num_anchors)
            # 倒也不是我们不看第二位，只是由于pred_obj的实现我们自己知道
            B, _, H, W = obj.shape
            obj = torch.permute(obj,(0, 2, 3, 1)).contiguous()
            obj = obj.view(B, -1)

            box = torch.permute(box,(0, 2, 3, 1)).contiguous()
            box = box.view(B, H, W, 4*self.num_anchors)
